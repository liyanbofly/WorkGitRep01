layui.define(['table', 'form', 'layer', 'jquery', 'element'], function (exports) {
    ...
    pca.init = function (province, city, area, initprovince, initcity, initarea, form,_id,filterprovince,filtercity,filterarea,proCallback,cityCallback,dictCallback) {//jQuery选择器, 省-市-区
        var form = layui.form;
        if (!province || !$(province).length) return;
        $(province).html('');
        $(province).append('<option value="" selected>全部</option>');
        for (var i in citys) {
        ...
        var _f_p = filterprovince ? filterprovince : 'province';
        var _p = _id ? "select(province-" + _id + ")" : "select("+_f_p+")";
        form.on(_p, function (data) {
        ...
        var _f_c = filtercity ? filtercity : 'city';
        var _c = _id ? "select(city-" + _id + ")" : "select("+_f_c+")";
        form.on(_c, function (data) {
            ...
            if ($.isFunction(cityCallback)) {
                cityCallback();
            }
        });

        var _f_a = filterarea ? filterarea : 'area';
        var _a = _id ? "select(area-" + _id + ")" : "select("+_f_a+")";
        form.on(_a, function (data) {
            pca.formHidden('area', data.value);
            //$('.pca-label-area').html(data.value);	//此处可以自己修改 显示的位置, 不想显示可以直接去掉
            if ($.isFunction(dictCallback)) {
                dictCallback();
            }
        });
        if (initprovince) $(province).next().find('[lay-value="' + initprovince + '"]').click();
