...
public class RedisUtil {
     ...
     *
     * @param moblie
     * @param operationType 手机验证码 操作类型(OprtsyionEnum) 枚举类型
     * @return ok:成功；0：失败
     */
    public String operationCode(String moblie, String operationType) {
        try {
            String code = MessageUtils.codeBuilder(4);
            String result = this.set(operationType + ":" + moblie, code, 300l);
           // Log.info(OprtsyionEnum.getValue(operationType) + "操作,手机号:{},验证码：{},保存redis结果:{}", moblie, code, result);
            if ("ok".equals(result.toLowerCase())) {
               // MessageUtils.sendMessage(moblie, "验证码：" + code + "，您正在进行" + OprtsyionEnum.getValue(operationType) + "操作，请在5分钟内按页面提示提交验证码，如非本人请忽略！");
            }
            return "ok";
        } catch (Exception e) {
     ...
     */
    public ResultVO validationCode(String moblie, String code, String operationType) {
        ResultVO resultVO = new ResultVO();
        try {
            if (ObjectUtil.isNullOrEmpty(code)) return resultVO.result(StatusCode.STATUS_0, "验证码为空！");
            return this.getKey(operationType + ":" + moblie, code);
        } catch (Exception e) {
            Log.error(e.getMessage(), e);
        }
        return resultVO.result(StatusCode.STATUS_0, "验证码已过期！");
    }
    ...
    public String set(String key, String value, Long expire) {
        ...
        try {
            ...
            //成功 返回OK 失败返回 0
            String ok = this.set(key, value, RedisConstants.datebase1);
            Long r = this.expire(key, expire.intValue(), RedisConstants.datebase1);
            return ok;
        } catch (Exception e) {
            Log.error(e.getMessage(), e);
        } finally {
     ...
     * @return
     */
    public UnionAuthVo getUnionAuth(String userId, int indexdb) {
        Jedis jedis = null;
        UnionAuthVo unionAuthVo = new UnionAuthVo();
        try {
            String dataStr = get(userId, indexdb);
            unionAuthVo = JsonUtil.fromJson(dataStr, UnionAuthVo.class);
        } catch (Exception e) {
            Log.error(e.getMessage(), e);
        } finally {
            closeResource(jedis);
     ...
     * @return
     */
    public ResultVO getKey(String key, String code) {
        Jedis jedis = null;
        ResultVO resultVO = new ResultVO();
        try {
            jedis = jedisPool.getResource();
            jedis.select(RedisConstants.datebase1);
            String value = jedis.get(key);
            Log.info("校验验证码,redid key:{},value:{}", key, value);
            if (ObjectUtil.isNullOrEmpty(code))
                return resultVO.result(StatusCode.STATUS_0, "验证码为空！");
            if (code.equals(value))
                return resultVO.result(StatusCode.STATUS_1, StatusCode.MSG_1);
        } catch (Exception e) {
            Log.error(e.getMessage(), e);
        } finally {
            closeResource(jedis);
    ...
    public String get(String key, int indexdb) {
        ...
        try {
            ...
            value = jedis.get(key);
            Log.info("redis get key:{},value:{}", key, !ObjectUtil.isNullOrEmpty(value) ? value.length() : value);
        } catch (Exception e) {
    ...
    public String set(String key, String value, int indexdb, int expire) {
        Jedis jedis = null;
        try {
            jedis = jedisPool.getResource();
            jedis.select(indexdb);
     ...
     * @return 返回删除成功的个数
     */
    public Long del(String keys, int indexdb) {
        Jedis jedis = null;
        try {
            jedis = jedisPool.getResource();
            jedis.select(indexdb);
